...
Statement::Statement(sqlite3 *db, sqlite3_stmt *stmt)
        : RefCount(), m_db(db), m_stmt(stmt) {
    IncRef();
}

Statement::Statement(const Statement &x)
        : RefCount(x), m_db(x.m_db), m_stmt(x.m_stmt) {
    IncRef();
}

bool Statement::Next(bool reset) {
    assert(m_stmt);
...
void Statement::DoBind(int i, const SQDB_STD_STRING &value) {
    const int ret =
            ...
            sqlite3_bind_text
                    ...
                    sqlite3_bind_text16
#endif
                    (m_stmt, i, value.c_str(), value.size() * sizeof(SQDB_STD_STRING::value_type),
                     SQLITE_TRANSIENT);
    CHECK(m_db, ret);
}
...
long long Statement::toAddr() {
    return (long long)m_stmt;
}
...
void Db::BeginTransaction() {
    Query(SQDB_MAKE_TEXT("BEGIN;")).Next(false);
}

void Db::CommitTransaction() {
    Query(SQDB_MAKE_TEXT("COMMIT;")).Next(false);
}

void Db::RollbackTransaction() {
    Query(SQDB_MAKE_TEXT("ROLLBACK;")).Next(false);
}
...
void Db::exec(const SQDB_CHAR *cmd) {
    const int ret = sqlite3_exec(m_db, cmd, NULL, NULL, NULL);
    CHECK(m_db, ret);
}

long long Db::toAddr() {
    return (long long) m_db;
}
